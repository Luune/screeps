...
module.exports.loop = function () {
    ...
    //----setup trade----
    var orderId1 = '6528f9dd2ffd9917b792206d'; //my 6528f9dd2ffd9917b792206d
    ...
    for (var roomName of myRooms) {
        ...
        if (order) {
            ...
            console.log(`Order ID: ${order.id}, Resource Type: ${order.resourceType},Room Name: ${order.roomName}, Price: ${order.price}, Amount: ${order.amount}, Remaining Amount: ${order.remainingAmount}, Trade Cost Energy: ${tradeCost} per ${tradeAmount}`);
        }
        ...
        if (totalWorkParts < sources.length * 5 && Game.rooms[roomName].controller.my) { //----harvester----
            if (enAvail >= 900) {
                ...
                Game.spawns[spawnName].spawnCreep([WORK, WORK, WORK, WORK, WORK, WORK, CARRY, MOVE, MOVE, MOVE, MOVE], newName, { memory: { role: 'harvester', loc: roomName } });
            } else if (enAvail >= 700) {
                var newName = 'Harvester' + roomName + Game.time;
                console.log('M>> Spawning new harvester: ' + newName);
            ...
            if (enAvail >= 1500 && sources.length > 1) {
                ...
                Game.spawns[spawnName].spawnCreep([WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE], newName, { memory: { role: 'upgrader', loc: roomName } });
            } else if (enAvail >= 900) {
                var newName = 'Upgrader' + roomName + Game.time;
                console.log('M>> Spawning new upgrader: ' + newName);
                Game.spawns[spawnName].spawnCreep([WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE], newName, { memory: { role: 'upgrader', loc: roomName } });
            } else if (enAvail >= 650) {
                var newName = 'Upgrader' + roomName + Game.time;
                console.log('M>> Spawning new upgrader: ' + newName);
                Game.spawns[spawnName].spawnCreep([WORK, WORK, WORK, WORK, CARRY, MOVE, MOVE, MOVE], newName, { memory: { role: 'upgrader', loc: roomName } });
            } else if (enAvail >= 500) {
                var newName = 'Upgrader' + roomName + Game.time;
                console.log('M>> Spawning new upgrader: ' + newName);
            ...
            if (enAvail >= 1600) {
                ...
                var newName = 'Builder' + roomName + Game.time;
                console.log('M>> Spawning new builder: ' + newName);
                Game.spawns[spawnName].spawnCreep([WORK, WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE], newName, { memory: { role: 'builder', loc: roomName } });
            }
            else if (enAvail >= 900) {
                var newName = 'Builder' + roomName + Game.time;
                console.log('M>> Spawning new builder: ' + newName);
            ...
            if (enAvail >= 1500) {
                ...
                Game.spawns[spawnName].spawnCreep([CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE], newName, { memory: { role: 'transporter', loc: roomName } });
        ...
        if (Game.rooms[roomName].find(FIND_HOSTILE_CREEPS).length > 0 && towerEn < 100) {
            if (fighters.length < 1) {
                if (enAvail >= 550) {
                    ...
                    Game.spawns[spawnName].spawnCreep([TOUGH, TOUGH, TOUGH, MOVE, MOVE, RANGED_ATTACK, MOVE, MOVE, RANGED_ATTACK], newName, { memory: { role: 'fighter', loc: roomName } });
        ...
        if (alchemists.length < 1 && labs.length > 0) {
            ...
            console.log('M>> Spawning new miner: ' + newName);
            Game.spawns[spawnName].spawnCreep([CARRY, CARRY, MOVE], newName, { memory: { role: 'alchemist', loc: roomName } });
        }
        ...
        for (var creepName in Game.creeps) {
            ...
            if (creep.memory.role == 'builder' && creep.memory.loc == roomName) {
                roleBuilder.run(creep, roomName, enAvail, storageStorage, dismantleTarget);
            }
            ...
            if (creep.memory.role == 'fighter' && creep.memory.loc == roomName) {
                roleFighter.run(creep, roomName, dismantleTarget);
            }
            ...
            if (creep.memory.role == 'alchemist' && creep.memory.loc == roomName) {
                roleAlchemist.run(creep, roomName, order, tradeAmount, tradeCost);
            }
        }
    } //----room wise end----
