...
var roleHarvester = {
    ...
    run: function (creep, roomName, sources) {
        ...
        else if (!creep.memory.fillLink ) {
            let roomLinks = Game.rooms[creep.memory.loc].find(FIND_MY_STRUCTURES, {
                filter: (structure) => {
                    return structure.structureType == STRUCTURE_LINK 
                    && structure.pos.inRangeTo(Game.getObjectById(creep.memory.workingSlot), 2)
                    && !Object.values(Game.flags).some(flag => flag.pos.isEqualTo(structure.pos));
                }
            });
            if (roomLinks.length > 0) {
                creep.memory.fillLink = roomLinks[0].id;
            }
            else {
                creep.memory.fillLink = '0';
            }
        }
        ...
        if (creep.room.name != creep.memory.loc) {
            ...
            if (creep.memory.discharging) {
                ...
                //---- store to link----
                // if (creep.memory.fillLink != '0') {
                //     let link = Game.getObjectById(creep.memory.fillLink);
            ...
            //----harvest----
                ...
                if (result == ERR_NOT_IN_RANGE) {
                    ...
                    delete creep.memory.fillLink;
                    delete creep.memory.path;
                    creep.memory.discharging = true;
                }

                // let workParts = _.filter(creep.body, (part) => part.type === WORK).length;
                // console.log(`creep身上共有 ${workParts} 个WORK部件`);
                if (creep.memory.fillLink != '0' && creep.store.getFreeCapacity(RESOURCE_ENERGY) <= creep.store.getCapacity() / 5) {
                    let link = Game.getObjectById(creep.memory.fillLink);
